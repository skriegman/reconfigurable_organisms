...
from tools.algorithms import ExhaustiveSearch
...
class MyGenotype(Genotype):
    def __init__(self):
        ...
        self.add_network(DirectEncoding(output_node_name="phase_offset", orig_size_xyz=IND_SIZE, symmetric=False),
                         freeze=True)
        self.add_network(DirectEncoding(output_node_name="material", orig_size_xyz=IND_SIZE, func=encoding,
...
if not os.path.isfile("./" + RUN_DIR + "/pickledPops/Gen_0.pickle"):
    ...
    my_optimization = ExhaustiveSearch(my_sim, my_env, my_pop, vox_types=[0, 1, 3])
